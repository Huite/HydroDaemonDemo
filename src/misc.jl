...
function read_weather_data(filename)
    ...
    df = CSV.read(filename, DataFrame, 
                  ...
                  silencewarnings=true)  # Silence warnings about type inference
...
df_transformed = transform(df_selected,
    :YYYYMMDD => ByRow(x -> Date(string(x), dateformat"yyyymmdd")) => :Date,
    :RH => ByRow(x -> coalesce(max(0, something(tryparse(Float64, x), -1)) / 10.0, 0)) => :P,
    :EV24 => ByRow(x -> coalesce(max(0, something(tryparse(Float64, x), -1)) / 10.0, 0)) => :ET
)
df = select(filter(row -> row.Date >= Dates.Date("1960-01-01"), df_transformed), [:Date, :P, :ET])
